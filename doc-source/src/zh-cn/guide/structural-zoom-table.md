...
`usingNumbers` 对整数参数按有符号数值精确比较，不区分整数位宽。
浮点参数按 `const*` 指令恢复的 32/64 位模式解释为 `float`/`double`，包括短立即数编码的零，
绝对误差小于 `1e-6` 时相等。整数运算的 `lit8`/`lit16` 操作数仅参与整数匹配。
同号无穷大相等，正负零相等，NaN 不匹配；不会据此推断源码中的真实类型。
